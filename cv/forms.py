...
class base_infoForm(ModelForm):
    class Meta:
        model = base_info
        exclude=('cuser','position','provience_applied','salary_annual','salary_monthly','status','remark','stage',)
    def clean_id_num(self):
        ...
        return id_num
class education_infoForm(ModelForm):
#     school=forms.ModelChoiceField(
#         queryset=schools.objects.all(),
#         widget=autocomplete.ModelSelect2(url=reverse('duals:schools-autocomplete')))
    class Meta:
        model = education_info
        fields = ['degree','school', 'major', 'start_date', 'end_date', 'activities','awards']
        widgets = {
